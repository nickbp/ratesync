...
void mpdtagger::mpd::Access::songs(std::list<mpdtagger::mpd::Song>& out) const {
	...
	while ((song = mpd_recv_song(conn)) != NULL) {
		out.push_back(Song(conn, mpd_song_get_uri(song)));
		mpd_song_free(song);
	}
}

const std::string& mpdtagger::mpd::Song::uri() const {
	return uri_;
}

bool mpdtagger::mpd::Song::rating(rating_t& out) const {
...
void mpdtagger::mpd::Song::rating_clear() {
...
void mpdtagger::mpd::Song::rating_set(rating_t rating) {
